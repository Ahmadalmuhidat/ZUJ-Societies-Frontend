        ...
        if (eventsRes.status === 'fulfilled' && eventsRes.value.status === 200) {
          ...
          const recommendedEvents = allEvents
            ...
            .map(event => ({
              ...
              date: event.Date ? (parseEventDate(event.Date)?.toISOString().split('T')[0] || 'TBD') : 'TBD',
              time: event.Time || 'TBD',
        ...
        {recommendations.map((item) => (
          <div key={item.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
            ...
            <div className="mt-3 flex space-x-2">
              ...
              <button className="px-3 py-2 text-xs text-gray-600 hover:text-gray-800 border border-gray-300 rounded-md hover:bg-gray-50 transition-colors">
                Learn More
              </button>
            </div>
          </div>
